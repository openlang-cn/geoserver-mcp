...
6. deploy/1panel/open-geoserver-mcp/ — 版本号通过 VERSION 表单字段由用户输入，docker-compose.yml 使用 `${VERSION}` 引用。升级版本时只需更新 data.yml 中 VERSION 的 default 值即可，无需创建新版本目录。

版本号修改完成后，必须运行 `pytest tests/test_package_version.py` 确认测试通过。

1Panel 安装包只需维护一个版本目录（当前为 1.0.9/），实际版本由用户安装时在 VERSION 字段中输入。升级时更新 data.yml 中 VERSION 的 default 值即可，无需创建新版本目录。
...
新增 MCP 工具时，如果 geoserver-rest 库不支持对应 API，需在 client.py 中通过 self.request 直接组装 REST API 调用。此类方法应标注 # Direct REST API 注释，并在 catalog.py 的 TOOLS 列表中按相同模式注册。
...
1. **MCP 服务层**：Codex 客户端 → MCP 服务的认证（如 OpenResty Basic Auth、反向代理），由部署基础设施处理，代码中不涉及。
2. **GeoServer 层**：MCP 服务 → GeoServer REST API 的认证，通过环境变量 GEOSERVER_USER / GEOSERVER_PASSWORD 注入，由 connection.py 创建 geo.Geoserver 实例时传入，库内部 _requests 方法自动附加 Basic Auth 头。

client.py 中 self.request 方法调用 self._geo._requests，后者的认证凭据来源于 connection.py 中读取的环境变量，而非 MCP 服务自身的认证。
